...
# @app.route("/image/get/<id>", methods=['POST'])
# @cross_origin()
# def image():
#     parameters = request.json
#     image = recolor_image(parameters['image'], parameters['method'], parameters['ellipse'],
#                                  parameters['confusionPoint'], parameters['luminances'])
#     dirpath = tempfile.mkdtemp(prefix=request.remote_addr)
#     image.save(dirpath + '/image_name.png')
#     send_file
...
def getConfusionPoint():
    ...
    linearLineCoef = -(angularLineCoef *
...
def differentiation():
    ...
    if (color_luminance_distance > luminance_distance):
        ...
        plot_transformed(ax, transformed_ellipse, color1Point,
                         color2Point, confusion_point)
